...
log_filepath = "/home/eloise/eloise/result_LAN/result-"+name+"/densenet121/data_1/log_"+name+".txt"
...
''' Add densenet121 BN2 layer data '''
Flag_BN2 = False
with open(log_filepath) as f:
    for line in f:
        text = line.strip().split()
...
''' Add densenet121 truncation layer data '''
Flag = False
with open(log_filepath) as f:
    for line in f:
        text = line.strip().split()
...
LR_folderPath = "/home/eloise/eloise/script/analysis/layer_onlyTime_LAN/predict_regression/LR_model/"
''' Predict conv '''
...
X_BN = pd.DataFrame()
X_BN['FLOPs'] = df_BN["BN_C"] * df_BN["BN_H"] * df_BN["BN_W"]
X_BN['IN_MACs'] = df_BN["BN_C"] * df_BN["BN_H"] * df_BN["BN_W"]
X_BN['PAR_MACs'] = df_BN["BN_C"]
X_BN['OUT_MACs'] = df_BN["BN_C"] * df_BN["BN_H"] * df_BN["BN_W"]
y_BN = df_BN['time_BN']
...
time_predict = sum(y_conv_pred) + sum(y_relu_pred) + sum(y_maxpool_pred) + sum(y_avgpool_pred) + sum(y_BN_pred) + sum(y_trunc_pred)
