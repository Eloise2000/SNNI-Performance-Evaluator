...
X_single['FLOPs'] = pd.Series([(2 * df_row["FC_CI"] * df_row["FC_CO"]) + df_row["FC_CO"]])
X_single['IN_MACs'] = pd.Series([df_row["FC_CI"] * 8])
X_single['PAR_MACs'] = pd.Series([(df_row["FC_CI"] + 1) * df_row["FC_CO"] * 8])
X_single['OUT_MACs'] = pd.Series([df_row["FC_CO"] * 8])

# X_single['FLOPs'] = (2 * df["FC_CI"] * df["FC_CO"]) + df["FC_CO"]
# X_single['IN_MACs'] = df["FC_CI"] * 8
# X_single['PAR_MACs'] = (df["FC_CI"] + 1) * df["FC_CO"] * 8
# X_single['OUT_MACs'] = df["FC_CO"] * 8
