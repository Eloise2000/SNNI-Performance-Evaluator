...
X = pd.DataFrame()
ap_H_output = (df['avgpool_H'] + df["avgpool_zPadHLeft"] + df["avgpool_zPadHRight"] - df["avgpool_ksizeH"]) / df["avgpool_strideH"] + 1
ap_W_output = (df['avgpool_W'] + df["avgpool_zPadWLeft"] + df["avgpool_zPadWRight"] - df["avgpool_ksizeW"]) / df["avgpool_strideW"] + 1
...
X['IN_MACs'] = (df['avgpool_H'] * df['avgpool_W']) * df["avgpool_C"] * 8
...
# plt.legend()
# plt.savefig('mape_ap_MoreThan1000.png')  # Provide the desired file name and extension
# plt.savefig('mape_ap_MoreThan100.png')  # Provide the desired file name and extension
# plt.savefig('mape_ap_all.png')  # Provide the desired file name and extension
