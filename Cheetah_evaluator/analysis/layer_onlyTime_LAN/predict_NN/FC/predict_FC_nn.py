...
tuner = RandomSearch(
    hypermodel,
    objective='val_loss',
    max_trials=50,  # Number of trials to perform
...
history = best_model.fit(x_train, y_train, epochs=200, batch_size=32, validation_data=(x_val, y_val))
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('FC_best_plot_' + name + '.png')

best_model.save('best_model_FC_' + name + '.keras')