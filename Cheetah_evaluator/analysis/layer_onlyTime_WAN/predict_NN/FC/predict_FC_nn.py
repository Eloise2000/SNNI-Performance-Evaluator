...
class MyHyperModel(HyperModel):
    ...
    def build(self, hp):
        ...
        for i in range(num_hidden_layers):
            ...
            model.add(layers.Dense(units=hp.Int(f"units_{i+1}", min_value=48, max_value=640, step=16), activation = 'relu'))
...
input_shape = (3,) # example input shape for avgpool
...
tuner = RandomSearch(
    ...
    project_name='240208-1'  # Name of the tuning project
)

# Search for the best hyperparameters
tuner.search(x_train, y_train, epochs=100, batch_size=32, validation_data=(x_val, y_val), verbose=2)  # Set verbose to 2
...
history = best_model.fit(x_train, y_train, epochs=EPOCH, batch_size=32, validation_data=(x_val, y_val))
...
plt.title('FC Training and Validation Loss for '+name)
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.savefig('FC_best_plot_' + name + '_' + str(EPOCH) + 'ep.png')

best_model.save('best_model_FC_' + name + '_' + str(EPOCH) + 'ep.keras')