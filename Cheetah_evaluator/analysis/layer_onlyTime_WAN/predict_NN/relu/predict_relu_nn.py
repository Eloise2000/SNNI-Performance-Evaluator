...
tuner.search(x_train, y_train, epochs=80, validation_data=(x_val, y_val), verbose=2)  # Set verbose to 2
...
plt.title('ReLU Training and Validation Loss for '+name)
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.savefig('relu_best_plot_' + name + '_' + str(EPOCH) + 'ep.png')

best_model.save('best_model_relu_' + name + '_' + str(EPOCH) + 'ep.keras')