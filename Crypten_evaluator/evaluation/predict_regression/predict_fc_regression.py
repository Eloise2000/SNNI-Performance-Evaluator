...
Author: Eloise Zhang
Date: 12 July 2024
Description: This script build analytical mdoel for Fully Connected Layer.
"""
...
'''physical operations'''
X['FLOPs'] = (2 * df["fc_CI"] * df["fc_CO"]) + df["fc_CO"]
X['IN_MACs'] = df["fc_CI"] * 8
X['PAR_MACs'] = (df["fc_CI"] + 1) * df["fc_CO"] * 8
X['OUT_MACs'] = df["fc_CO"] * 8
...
''' Dump the result '''
dump(reg, folder_path + 'fc_' + name + '_LR_model.joblib')
dump(scaler, folder_path + 'fc_' + name + '_scaler.joblib')
...
'''physical operations'''
X_single['FLOPs'] = [(2 * df_row["fc_CI"] * df_row["fc_CO"]) + df_row["fc_CO"]]
X_single['IN_MACs'] = [df_row["fc_CI"] * 8]
X_single['PAR_MACs'] = [(df_row["fc_CI"] + 1) * df_row["fc_CO"] * 8]
X_single['OUT_MACs'] = [df_row["fc_CO"] * 8]
