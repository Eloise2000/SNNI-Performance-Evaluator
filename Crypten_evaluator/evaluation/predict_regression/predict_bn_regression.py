...
Author: Eloise Zhang
Date: 12 July 2024
Description: This script build analytical mdoel for BatchNormalization Layer.
"""
...
'''physical operations'''
X['FLOPs'] = df["bn_C"] * df["bn_H"] * df["bn_W"]
X['IN_MACs'] = df["bn_C"] * df["bn_H"] * df["bn_W"]
X['PAR_MACs'] = df["bn_C"]
X['OUT_MACs'] = df["bn_C"] * df["bn_H"] * df["bn_W"]
...
reg = LinearRegression(positive=True, fit_intercept=True).fit(X_normalized, y)
...
'''physical operations'''
X_single['FLOPs'] = [df_row["bn_C"] * df_row["bn_H"] * df_row["bn_W"]]
X_single['IN_MACs'] = [df_row["bn_C"] * df_row["bn_H"] * df_row["bn_W"]]
X_single['PAR_MACs'] = [df_row["bn_C"]]
X_single['OUT_MACs'] = [df_row["bn_C"] * df_row["bn_H"] * df_row["bn_W"]]
