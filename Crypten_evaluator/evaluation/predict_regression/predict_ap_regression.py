...
Author: Eloise Zhang
Date: 12 July 2024
Description: This script build analytical mdoel for AvgPool Layer.
"""
...
'''
avgpool columns= "ap_N", "ap_HI", "ap_WI", "ap_CI", "ap_HO", "ap_WO", "ap_CO", 
...
'''physical operations'''
X['FLOPs'] = (df["ap_F"] * df["ap_F"]) * (df["ap_HO"] * df["ap_WO"]) * df["ap_CO"]
# sizeof(uint64_t) is unsigned 64-bit integer -> 8 bytes
X['IN_MACs'] = (df['ap_HI'] * df['ap_WI']) * df["ap_CI"] * 8
X['OUT_MACs'] = (df["ap_HO"] * df["ap_WO"]) * df["ap_CO"] * 8
...
'''physical operations'''
X_single['FLOPs'] = [(df_row["ap_F"] * df_row["ap_F"]) * (df_row["ap_HO"] * df_row["ap_WO"]) * df_row["ap_CO"]]
X_single['IN_MACs'] = [(df_row['ap_HI'] * df_row['ap_WI']) * df_row["ap_CI"] * 8]
X_single['OUT_MACs'] = [(df_row["ap_HO"] * df_row["ap_WO"]) * df_row["ap_CO"] * 8]
