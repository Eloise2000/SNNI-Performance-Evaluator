...
Author: Eloise Zhang
Date: 12 July 2024
Description: This script build analytical mdoel for MaxPool Layer.
"""
...
'''
maxpool columns= "mp_N", "mp_HI", "mp_WI", "mp_CI", "mp_HO", "mp_WO", "mp_CO", 
...
'''physical operations'''
X['FLOPs'] = (df["mp_F"] * df["mp_F"]) * (df["mp_HO"] * df["mp_WO"]) * df["mp_CO"]
# sizeof(uint64_t) is unsigned 64-bit integer -> 8 bytes
X['IN_MACs'] = (df['mp_HI'] * df['mp_WI']) * df["mp_CI"] * 8
X['OUT_MACs'] = (df["mp_HO"] * df["mp_WO"]) * df["mp_CO"] * 8
...
'''physical operations'''
X_single['FLOPs'] = [(df_row["mp_F"] * df_row["mp_F"]) * (df_row["mp_HO"] * df_row["mp_WO"]) * df_row["mp_CO"]]
X_single['IN_MACs'] = [(df_row['mp_HI'] * df_row['mp_WI']) * df_row["mp_CI"] * 8]
X_single['OUT_MACs'] = [(df_row["mp_HO"] * df_row["mp_WO"]) * df_row["mp_CO"] * 8]
